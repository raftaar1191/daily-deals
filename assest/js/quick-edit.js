...
jQuery(function( $ ) {
	$( '#the-list' ).on( 'click', '.editinline', function() {
		...
		var sku            = $wc_inline_data.find( '.sku' ).text(),
			...
			HARIMAY_DD_get_product_id_for_transient_id      	= $wc_inline_data.find( '.HARIMAY_DD_get_product_id_for_transient_id' ).text(),
			HARIMAY_DD_daily_deals_price       			= $wc_inline_data.find( '.HARIMAY_DD_daily_deals_price' ).text(),
			HARIMAY_DD_daily_deals_quantity       			= $wc_inline_data.find( '.HARIMAY_DD_daily_deals_quantity' ).text(),
			HARIMAY_DD_daily_deals_order       			= $wc_inline_data.find( '.HARIMAY_DD_daily_deals_order' ).text(),
		...
		if ( 'yes' === HARIMAY_DD_get_product_id_for_transient_id ) {
			$( 'input[name="HARIMAY_DD_get_product_id_for_transient_id"]', '.inline-edit-row' ).attr( 'checked', 'checked' );
		} else {
			$( 'input[name="HARIMAY_DD_get_product_id_for_transient_id"]', '.inline-edit-row' ).removeAttr( 'checked' );
		}
		
		$( 'input[name="HARIMAY_DD_daily_deals_price"]', '.inline-edit-row' ).val( HARIMAY_DD_daily_deals_price );
		$( 'input[name="HARIMAY_DD_daily_deals_quantity"]', '.inline-edit-row' ).val( HARIMAY_DD_daily_deals_quantity );
		$( 'input[name="HARIMAY_DD_daily_deals_order"]', '.inline-edit-row' ).val( HARIMAY_DD_daily_deals_order );
	});
...
jQuery( function( $ ) {
	...
	( '.datepicker_dates_fields', wrapper ).each( function() {
		var dates = $( this ).find( '.datepicker_dates_fields input' ).datepicker({
			defaultDate:     '',
			dateFormat:      'yy-mm-dd',
...
jQuery( function( $ ){
	if( jQuery( '.add_datepicker_harimay' ).size() > 0 ){
		$( '.add_datepicker_harimay' ).datepicker({
			defaultDate:     '',
			dateFormat:      'yy-mm-dd',
			showButtonPanel: true
		});
